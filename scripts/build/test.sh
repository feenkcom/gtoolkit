...
xvfb-run -a -e /dev/stdout ./pharo-ui Pharo.image examples --junit-xml-output 'GToolkit-.*' 'GT4SmaCC-.*' 'DeepTraverser-.*' 'Brick' 'Brick-.*' 'Bloc' 'Bloc-.*' 'Starta-.*' 2>&1
xvfb-run -a -e /dev/stdout ./pharo-ui Pharo.image gtexportreport --report=GtGtoolkitArchitecturalReport
